...
- **Search/filters are client-side**, filtering the already-fetched list in the browser (search input + status `Select`), not new server-side query parameters. Election/candidate counts in this app are small enough that this is simpler than pagination-aware server-side search and leaves every existing list query untouched.
...
- **Admin elections table** (`elections-table.tsx`): search input (title, case-insensitive substring) + status `Select` (All / Draft / Upcoming / Active / Ended / Closed, matching `getEffectiveStatus`'s values). Filtering happens before the table's existing pagination; changing a filter resets to page 1.
